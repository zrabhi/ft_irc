...
void    Commands::broadcasParttMessage(Iterator _client, Channel &channel)
{
    Vector_map users;
    ...
    for (Vector_map::iterator _it = users.begin(); _it != users.end(); _it++)
    {
        ...
        for (; clients != _it->second.end() ; clients++)
            replyto(REPLY_PART(_client->second.getNickName(), _client->second.getNickName()[0], channel.getName()), clients->first);
    }
}
...
bool Commands::PART(Vector params, Iterator &_client)
{
    ...
    for (size_t i = 0; i < channelNames.size(); i++)
    {
        ...
        broadcasParttMessage(_client, channel);
    }
    return (true);
}